...
final class VungleAdapter: PartnerAdapter {
    ...
    func setConsents(_ consents: [ConsentKey: ConsentValue], modifiedKeys: Set<ConsentKey>) {
        ...
        // See https://support.vungle.com/hc/en-us/articles/360048572411
        // Ignore if the consent status has been directly set by publisher via the configuration class.
        if !VungleAdapterConfiguration.isCCPAStatusOverridden && modifiedKeys.contains(ConsentKeys.ccpaOptIn) {
    ...
    func mapSetUpError(_ error: Error) -> ChartboostMediationError.Code? {
        ...
        case 104:
            // The status code from an API call (to endpoints such as config, ads, etc.) returned something outside of the 2xx.
        ...
        case 122: // The ads endpoint doesn't exist in the config response body.
            return .initializationFailureNetworkingError
        case 123:
            // The RI (Incentivized Reporting) endpoint is missing from the config response body. This endpoint is used in conjunction with
            // rewarded (incentivized) ads.
            return .initializationFailureInvalidAppConfig
        default:
            return nil
        }
    ...
    func mapLoadError(_ error: Error) -> ChartboostMediationError.Code? {
        ...
        case 104:
            // The status code from an API call (to endpoints such as config, ads, etc.) returned something outside of the 2xx range.
            // Message contains the URL.
        ...
        case 126: // During ad loading, an asset failed to download due to insufficient space available on the user’s device.
            return .loadFailureOutOfStorage
        case 127:
            // During ad loading, an asset failed to download because the space required to download and cache the asset exceeded the
            // maximum space available.
            return .loadFailureOutOfStorage
        case 130: // MRAID JavaScript file download failed.
            return .loadFailureNetworkingError
        case 131:
            // Failed to save MRAID JavaScript files to disk.
...
extension String {
    ...
    fileprivate static let appIDKey = "vungle_app_id"
}
